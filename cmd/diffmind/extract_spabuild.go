...
// extractorFindSPAWorkspace walks up from cwd looking for a directory that
...
func extractorFindSPAWorkspace() string {
	cwd, err := os.Getwd()
	if err != nil {
		return ""
	}
	for dir := cwd; ; {
		candidate := filepath.Join(dir, "internal", "extractor", "ui", "web", "package.json")
...
// extractorSPARebuildNeeded returns true when ANY file under the SPA source
...
// false and "" otherwise.
func extractorSPARebuildNeeded(webDir string) (bool, string) {
	distDir := filepath.Join(webDir, "dist")
	if _, err := os.Stat(distDir); err != nil {
		return true, "dist/ does not exist"
	}
	oldestDist, ok := extractorOldestMtimeIn(distDir)
	if !ok {
	...
	for _, p := range srcRoots {
		newest, why, ok := extractorNewestMtimeIn(p)
		if !ok {
			continue
		}
...
// extractorNewestMtimeIn returns the latest modification time found anywhere
...
// doesn't exist.
func extractorNewestMtimeIn(root string) (time.Time, string, bool) {
...
// extractorOldestMtimeIn returns the earliest modification time found anywhere
// under root. Used to determine whether dist/ is fully fresh or just
// partially regenerated. ok=false means root is empty.
func extractorOldestMtimeIn(root string) (time.Time, bool) {
...
// extractorEnsureSPABuilt runs `npm run build` in webDir when the SPA sources
...
func extractorEnsureSPABuilt(skip bool) string {
	webDir := extractorFindSPAWorkspace()
	if webDir == "" {
	...
	need, reason := extractorSPARebuildNeeded(webDir)
	if !need {
