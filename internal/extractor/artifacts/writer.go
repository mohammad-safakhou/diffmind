...
func Write(in WriteInput) (string, error) {
	...
	if err := writeProtocolArtifacts(runDir, in, manifest); err != nil {
		return "", err
	}
...
func writeProtocolArtifacts(runDir string, in WriteInput, manifest model.RunManifest) error {
	doc, err := buildProtocol(in, manifest)
	if err != nil {
		return err
	}
	jsonPath := filepath.Join(runDir, ProtocolServiceJSON)
	...
	yf, err := os.Create(filepath.Join(runDir, ProtocolServiceYAML))
	if err != nil {
		return err
	}
