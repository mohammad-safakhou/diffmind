...
func TestRunMatchesRepo(t *testing.T) {
	...
	if !RunMatchesRepo(DiffMindRunInfo{ServiceID: "service.checkout_service"}, "checkout-service", "repo-2", "/new/checkout/service") {
		t.Fatal("expected Protocol service identity to match repo")
	}
