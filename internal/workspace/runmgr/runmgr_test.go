...
func setup(t *testing.T) (*Manager, string, []store.RunRepoRef) {
	...
	beRuns := t.TempDir()
	copyDir(t, filepath.Join(root, "testdata", "workspace", "sample_diffmind_output", "order-service", ".diffmind", "runs", "run_001"), filepath.Join(beRuns, "order-run"))
	copyDir(t, filepath.Join(root, "testdata", "workspace", "sample_diffmind_output", "billing-service", ".diffmind", "runs", "run_001"), filepath.Join(beRuns, "billing-run"))
	...
	orderRepo, _ := st.CreateRepo(p.ID, store.Repo{Name: "order-service", Path: filepath.Join(root, "testdata", "workspace", "sample_service_repos", "order-service"), Kind: "service_repo"})
	billingRepo, _ := st.CreateRepo(p.ID, store.Repo{Name: "billing-service", Path: filepath.Join(root, "testdata", "workspace", "sample_service_repos", "billing-service"), Kind: "service_repo"})
