...
import (
	...
	ag "github.com/mohammad-safakhou/diffmind/internal/workspace/archgraph"
)

func TestBuildArchitectureGraphUsesProtocolTargetsAndResourceKinds(t *testing.T) {
	root := t.TempDir()
	atsRun := filepath.Join(root, "ats")
	boostRun := filepath.Join(root, "boost")
	writeProtocolRun(t, atsRun, `{
...
`), 0o644); err != nil {
	...
	writeProtocolRunWithRepoPath(t, atsRun, atsRepo, `{
  "schema": "diffmind.service.v1",
  "service": {"id": "routing-service", "name": "routing-service"},
  "objects": {},
  "flows": [],
...
func TestBuildArchitectureGraphResolvesServiceReferences(t *testing.T) {
	root := t.TempDir()
	...
	writeProtocolRun(t, contentStoreRun, `{
  "schema": "diffmind.service.v1",
  "service": {"id": "content-store", "name": "content-store"},
  "objects": {},
  "flows": [],
  "observations": [],
  "evidence": []
}`)
	writeProtocolRun(t, mediaStoreRun, `{
  "schema": "diffmind.service.v1",
  "service": {"id": "media-store", "name": "media-store"},
  "objects": {},
  "flows": [],
  "observations": [],
  "evidence": []
}`)
	writeProtocolRun(t, catalogueRun, `{
  "schema": "diffmind.service.v1",
  "service": {"id": "catalog-management-api", "name": "catalog-management-api"},
  "objects": {},
  "flows": [],
  "observations": [],
  "evidence": []
  ...
  "objects": {
    "http_calls": [
      ...
      },
      {
        ...
        "method": "GET",
        "target": {"type": "service", "ref": "service.media-store", "unresolved": false},
        "status": "confirmed",
        "confidence": "high",
        "origin": "deterministic"
      },
  ...
  "evidence": []
	...
	graph := buildArchitectureGraph("run-1", map[string]string{
		"content-store":          contentStoreRun,
		"media-store":            mediaStoreRun,
		"catalog-management-api": catalogueRun,
		"caller":                 callerRun,
	})
	expected := map[string]bool{
		"content-store":          false,
		"media-store":            false,
		"catalog-management-api": false,
	}
	for _, edge := range graph.Edges {
...
func TestBuildArchitectureGraphCanonicalizesQueueNodeKeys(t *testing.T) {
	...
	writeProtocolRun(t, publisherRun, `{
  "schema": "diffmind.service.v1",
  ...
  "evidence": []
	...
	writeProtocolRun(t, consumerRun, `{
  "schema": "diffmind.service.v1",
...
func TestBuildArchitectureGraphRendersProtocolRPCCalls(t *testing.T) {
	root := t.TempDir()
	callerRun := filepath.Join(root, "game-service")
	metadataRun := filepath.Join(root, "metadata")
	writeProtocolRun(t, metadataRun, `{
  "schema": "diffmind.service.v1",
...
func TestBuildArchitectureGraphRendersWorkflowOrchestration(t *testing.T) {
  ...
  "evidence": []
	...
	writeProtocolRun(t, payloadBuilderRun, `{
  "schema": "diffmind.service.v1",
...
func TestArchitectureGraphCarriesProtocolFlowDetails(t *testing.T) {
	root := t.TempDir()
  ...
  "evidence": []
	...
	if conns[0].Kind != "request_flow" || conns[0].DataDependencies == nil || conns[0].Edges == nil {
		t.Fatalf("expected Protocol flow details in connection summary, got %+v", conns[0])
	}
	...
	if got := graph.Services[0].HTTPRoutes[0]; got.ID != "http.create_order" || got.Kind != "http_endpoint" {
		t.Fatalf("expected entity summary to expose Protocol id/kind, got %+v", got)
	}
}
...
func writeProtocolRun(t *testing.T, runDir, body string) {
	writeProtocolRunWithRepoPath(t, runDir, "", body)
}
...
func writeProtocolRunWithRepoPath(t *testing.T, runDir, repoPath, body string) {
	t.Helper()
