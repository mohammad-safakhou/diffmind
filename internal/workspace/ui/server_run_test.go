...
func TestGraphRunOverHTTP(t *testing.T) {
	...
	order, _ := st.CreateRepo(p.ID, store.Repo{Name: "order-service", Path: filepath.Join(root, "testdata", "workspace", "sample_service_repos", "order-service"), Kind: "service_repo"})
	billing, _ := st.CreateRepo(p.ID, store.Repo{Name: "billing-service", Path: filepath.Join(root, "testdata", "workspace", "sample_service_repos", "billing-service"), Kind: "service_repo"})
