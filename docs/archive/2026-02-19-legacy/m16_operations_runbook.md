...
Operational mutation endpoints (requires ops mutation permissions, e.g. `tenant_admin` or `ops:write`):
- `POST /ops/backup`
- `POST /ops/restore`
- `POST /ops/rollout`
- `POST /ops/drill`
...
`POST /ops/drill` runs a full operational drill bundle:
1. backup
2. restore
3. rollout plan generation
4. SLO evaluation

Response contains deterministic drill artifact paths and per-step error fields.
