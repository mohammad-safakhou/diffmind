...
//
// plus the marker files findSPAWorkspace looks for.
...
func TestSPARebuildNeeded_SrcNewerThanDist(t *testing.T) {
	...
	need, reason := spaRebuildNeeded(web)
	if !need {
...
func TestSPARebuildNeeded_EmptyDist(t *testing.T) {
	...
	need, _ := spaRebuildNeeded(web)
	if !need {
		t.Fatalf("empty dist/ must trigger a rebuild")
	}
}

// findSPAWorkspace walks up from cwd looking for the workspace. We
