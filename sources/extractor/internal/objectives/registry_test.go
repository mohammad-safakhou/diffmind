...
func TestDefaultOrderAndCount(t *testing.T) {
	...
	for i, want := range wantIDs {
		...
		if got[i].Type == "" || got[i].Description == "" {
			t.Errorf("objective[%d] (%s) missing Type or Description", i, got[i].ID)
		}
	}
}

// TestDefaultMetaApplied verifies Default() overlays deterministic detail keys
// onto every objective type.
