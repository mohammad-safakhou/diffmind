...
// hintsFor returns the objective's AST hints for the orchestrator's current
// index and subDir, honouring the DiscoveryASTHints config flag (empty hints
// when disabled → prompts are byte-identical to the pre-grounding behaviour).
// fileScope optionally restricts hints to a shard's directories.
func (o *orchestrator) hintsFor(obj objectives.Objective, fileScope []string) objectiveHints {
	if !o.cfg.Runtime.DiscoveryASTHints {
		return objectiveHints{}
	}
	return buildObjectiveHints(o.astIndex, obj, o.subDir, fileScope)
}
