...
type Output struct {
	Results []extraction.DetailResult
}
...
// Run executes Stage 3. Detail enrichment turns each
...
func (r Runner) Run(ctx context.Context, input Input) Output {
	results := r.run(ctx, input.Jobs, input.RepoFacts, input.Progress)
	return Output{Results: results}
}

func (r Runner) run(ctx context.Context, jobs []detailJob, rf *repoFacts, onResult func()) []detailResult {
	if len(jobs) == 0 {
		return nil
	}
	...
	if r.RunDir != "" && r.Store != nil {
		checkpoint = r.Store.LoadDetailCheckpoint(filepath.Join(r.RunDir, runstate.StateDir))
	}

	pending, carriedResults, carriedMessages := r.partitionDetailJobs(jobs, checkpoint)
	r.emitCarriedSkips(carriedResults, carriedMessages, onResult)
	...
	r.emitBatchPlaceholders(batches)
	return r.runDetailWorkers(ctx, batches, rf, carriedResults, onResult)
}
...
func (r Runner) partitionDetailJobs(jobs []detailJob, checkpoint map[string]runstate.DetailCheckpointEntry) (pending []detailJob, carried []detailResult, carriedMessages map[string]string) {
	...
	for _, j := range jobs {
		...
		if !ok {
			if extraction.IsCompleteDeterministicSeed(j.Objective, &j.Seed) {
				seed := j.Seed
				...
				if checkpointEntry, ok := r.CheckpointForSeed(j); ok && r.Store != nil {
					r.Store.AppendDetailEntity(checkpointEntry)
				}
			} else {
...
func (r Runner) emitCarriedSkips(carried []detailResult, carriedMessages map[string]string, onResult func()) {
	for _, result := range carried {
		jobID := "detail." + result.Objective.ID + "." + extraction.SafeJobID(result.SeedName)
		key := runstate.DetailEntityKey(result.Objective.ID, result.SeedName)
		message := carriedMessages[key]
		if message == "" {
			message = "resumed from per-entity checkpoint"
		}
		runnerEmit := events.Event{
			Kind: events.KindJobCompleted, Stage: "detail", JobID: jobID, Status: events.StatusSkipped,
			Message:  message,
			ParentID: "discover." + result.Objective.ID,
			Payload: map[string]any{
				"objective_id":  result.Objective.ID,
				"name":          result.SeedName,
...
func (r Runner) emitBatchPlaceholders(batches [][]detailJob) {
	for _, b := range batches {
		...
		for _, j := range b {
			jobID := "detail." + j.Objective.ID + "." + extraction.SafeJobID(j.Seed.Name)
			r.emit(events.Event{
...
func (r Runner) runDetailWorkers(ctx context.Context, batches [][]detailJob, rf *repoFacts, carried []detailResult, onResult func()) []detailResult {
	workers := r.Workers
	if workers <= 0 {
		workers = 8
	}
	...
	for i := 0; i < workers; i++ {
		...
		go func(workerID int) {
			...
			for batch := range batchCh {
				...
				results, err := r.runDetailBatchOne(stageCtx, batch, rf)
				if err != nil {
				...
				for _, result := range results {
					r.checkpointDetailResult(result)
					resCh <- result
				}
			}
		}(i + 1)
...
func (r Runner) checkpointDetailResult(result detailResult) {
	if result.Err != nil || r.Store == nil {
		return
	}
	...
	if result.Item == nil {
		entry.Skipped = true
		r.Store.AppendDetailEntity(entry)
		return
	}
	...
	base, ur := extraction.ToBase(r.RepoPath, result.Objective, *result.Item, r.MinConfidence)
	if ur != nil {
	...
	if result.Objective.Kind == model.KindExposure {
...
func (r Runner) runDetailBatchOne(ctx context.Context, batch []detailJob, rf *repoFacts) ([]detailResult, error) {
	if len(batch) == 0 {
		return nil, nil
	}
	...
	started := time.Now()
	r.emitBatchStarted(batch, batchJobID, obj)

	hints := extraction.ObjectiveHints{}
	if r.Hints != nil {
		hints = r.Hints(obj)
	}
	prompt := extraction.BuildDetailBatchPrompt(obj, seeds, rf, r.SubDir, hints)
	schema := extraction.EntityListSchemaForObjective(obj)
	payload, err := r.Prompt(ctx, batchJobID, prompt, schema)
	dur := time.Since(started)
	if err != nil {
		r.emitBatchFailed(batch, batchJobID, obj, err, dur)
		return nil, err
	}
	...
	if len(items) == 0 {
		if single, ok := payload["item"]; ok && single != nil {
			if it := extraction.ParseSingleEntity(single); it != nil {
				items = []llmEntity{*it}
			}
		}
	}
	...
	results := r.assembleBatchResults(batch, batchJobID, items, dur)
	r.emitBatchCompleted(batch, batchJobID, obj, results, dur)
	return results, nil
}
...
func (r Runner) emitBatchStarted(batch []detailJob, batchJobID string, obj objectives.Objective) {
	...
	for _, j := range batch {
		entityJobID := "detail." + j.Objective.ID + "." + extraction.SafeJobID(j.Seed.Name)
		r.emit(events.Event{
...
func (r Runner) emitBatchFailed(batch []detailJob, batchJobID string, obj objectives.Objective, err error, dur time.Duration) {
	r.emit(events.Event{
...
func (r Runner) assembleBatchResults(batch []detailJob, batchJobID string, items []llmEntity, dur time.Duration) []detailResult {
	...
	for i, j := range batch {
		...
		if i < len(items) {
			...
			extraction.ForceObjectiveType(j.Objective, &it)
...
func (r Runner) emitBatchCompleted(batch []detailJob, batchJobID string, obj objectives.Objective, results []detailResult, dur time.Duration) {
...
func detailBatchJobID(batch []detailJob) string {
	...
	return "detail." + batch[0].Objective.ID + ".batch." + extraction.SafeJobID(first)
}
...
func batchDisplayName(batch []detailJob) string {
	...
	if len(batch) > preview {
		out += " (+" + extraction.Itoa(len(batch)-preview) + " more)"
	}
	return out
}
...
func (r Runner) CheckpointForSeed(job extraction.DetailJob) (runstate.DetailCheckpointEntry, bool) {
	base, unresolved := extraction.ToBase(r.RepoPath, job.Objective, job.Seed, r.MinConfidence)
	if unresolved != nil {
		return runstate.DetailCheckpointEntry{}, false
	}
	entry := runstate.DetailCheckpointEntry{
		Key:         runstate.DetailEntityKey(job.Objective.ID, job.Seed.Name),
		ObjectiveID: job.Objective.ID,
		SeedName:    job.Seed.Name,
	}
	if job.Objective.Kind == model.KindExposure {
		exposure := model.Exposure{BaseEntity: base}
		entry.Exposure = &exposure
	} else {
		dependency := model.Dependency{BaseEntity: base}
