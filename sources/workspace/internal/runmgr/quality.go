...
func (m *Manager) architectureStats(pid string, manifest store.RunManifest) architectureRunStats {
	...
	for _, ref := range manifest.Repos {
		...
		if doc, err := artifacts.ReadDiffMind protocol(runPath); err == nil {
			if doc.Repository.Dirty {
				stats.quality.DirtyRepos++
			}
			stats.quality.MissingEvidenceObjects += countDiffMind protocolMissingEvidence(doc)
		}
	}
...
func countDiffMind protocolMissingEvidence(doc *protocol.Document) int {
	if doc == nil {
		return 0
	}
	missing := 0
...
func graphQualityWarnings(q store.GraphQuality) []string {
	...
	if q.MissingEvidenceObjects > 0 {
		warnings = append(warnings, fmt.Sprintf("%d DiffMind protocol objectives are missing observation or evidence refs; improve detector evidence or mark imported/config-derived facts explicitly", q.MissingEvidenceObjects))
	}
	if q.StaleRepos > 0 {
